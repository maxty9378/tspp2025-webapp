-- Fix task completions table
ALTER TABLE task_completions
ALTER COLUMN id TYPE TEXT,
ALTER COLUMN id SET DEFAULT gen_random_uuid()::text;

-- Drop existing policies
DROP POLICY IF EXISTS "aos_responses_select_v4" ON aos_responses;
DROP POLICY IF EXISTS "aos_responses_insert_v4" ON aos_responses;
DROP POLICY IF EXISTS "aos_responses_update_v4" ON aos_responses;

-- Create more permissive policies for aos_responses
CREATE POLICY "aos_responses_select_v5"
ON aos_responses FOR SELECT
USING (true);

CREATE POLICY "aos_responses_insert_v5"
ON aos_responses FOR INSERT
WITH CHECK (true);

CREATE POLICY "aos_responses_update_v5"
