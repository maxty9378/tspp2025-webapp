...
BEGIN
  ...
  IF NEW.liked_by IS NOT NULL THEN
    ...
    LOOP
      ...
      -- Update the liker's likes array
      UPDATE users
      SET likes = array_remove(COALESCE(likes, ARRAY[]::TEXT[]), message_user_id);

      UPDATE users
      SET likes = array_append(COALESCE(likes, ARRAY[]::TEXT[]), message_user_id)
      WHERE id = liker_id
      AND NOT (COALESCE(likes, ARRAY[]::TEXT[]) @> ARRAY[message_user_id]);
    END LOOP;
  END IF;
