...
DROP FUNCTION IF EXISTS add_like(TEXT, TEXT);

-- Create improved function to handle likes across messages and profiles
...
BEGIN
  ...
  -- Calculate total likes (profile likes + message likes)
  SELECT COUNT(*) INTO total_likes
  FROM messages
  WHERE user_id = target_user_id AND liked_by ? liker_id;
